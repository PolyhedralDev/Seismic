...
import com.polyhedraldevelopment.seismic.algorithms.samplers.DerivativeNoiseSampler;
    ...
    public double[] erosion(int seed, double x, double y, double dirX, double dirY) {
        ...
        for(int cellX = gridX - 1; cellX <= gridX + 1; cellX++) {
            for(int cellY = gridY - 1; cellY <= gridY + 1; cellY++) {
                ...
                cumAmp += amp;
                double directionalStrength = LinearAlgebraFunctions.dotProduct(cellOriginDeltaX, cellOriginDeltaY, dirX, dirY) * TAU;
                noise += TrigonometryFunctions.cos(directionalStrength) * amp;
                double sinAngle = TrigonometryFunctions.cos(directionalStrength) * amp;
