...
public class CubicSpline {
     ...
     */
    public static double calculate(double in, @NotNull double[] fromValues, double[] toValues, double[] gradients) {
        ...
        if(pointIdx < 0) { // If to left of first point return linear function intersecting said point using point's gradient
            ...
            return InterpolationFunctions.lerp(toLeft, toRight, t) + t * (1.0F - t) * InterpolationFunctions.lerp(
                gradientLeft * fromDelta - toDelta,
                -gradientRight * fromDelta + toDelta, t);
        }
    }

    private static int floorBinarySearch(double targetValue, @NotNull double[] values) {
        int left = 0;
