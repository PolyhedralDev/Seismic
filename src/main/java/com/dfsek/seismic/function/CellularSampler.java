...
import com.dfsek.seismic.function.simplex.OpenSimplex2Sampler;
 ...
 *
 * @author FastNoiseLite contributors, dfsek
 */
public class CellularSampler extends NoiseFunction {
    ...
    public double getNoiseRaw(int seed, double x, double y) {
                ...
                for(int xi = xr - 1; xi <= xr + 1; xi++) {
                    ...
                    for(int yi = yr - 1; yi <= yr + 1; yi++) {
                        ...
                        if(newDistance < distance0) {
                            ...
                            closestHash = hash;
                            center.x = (xi + RAND_VECS_2D[idx] * cellularJitter) / frequency;
                            center.y = (yi + RAND_VECS_2D[idx | 1] * cellularJitter) / frequency;
        ...
        switch(returnType) {
            ...
            case NoiseLookup:
                return noiseLookup.getNoise(center.x, center.y);
            case Distance3:
                return distance2 - 1;
            case Distance3Add:
    ...
    public double getNoiseRaw(int seed, double x, double y, double z) {
                    ...
                    for(int yi = yr - 1; yi <= yr + 1; yi++) {
                        ...
                        for(int zi = zr - 1; zi <= zr + 1; zi++) {
                            ...
                            if(newDistance < distance0) {
                                distance0 = newDistance;
                                closestHash = hash;
                                center.x = (xi + RAND_VECS_3D[idx] * cellularJitter) / frequency;
                                center.y = (yi + RAND_VECS_3D[idx | 1] * cellularJitter) / frequency;
                                center.z = (zi + RAND_VECS_3D[idx | 2] * cellularJitter) / frequency;
                            } else if(newDistance < distance1) {
                                distance2 = distance1;
                                distance1 = newDistance;
                            } else if(newDistance < distance2) {
        ...
        switch(returnType) {
            ...
            case Distance2Div:
                return distance0 / distance1 - 1;
            case NoiseLookup:
                return noiseLookup.getNoise(center.x, center.y, center.z);
            case Distance3:
                return distance2 - 1;
            case Distance3Add:
                return (distance2 + distance0) * 0.5 - 1;
